...
class TdbAttr {
  ...
  Future<void> _initSdk() async {
    ...
    final devKey = TdbConfig.installKey;
    tdbLog('[attr] warmup devKeyLen=${devKey.length}');
    if (devKey.isEmpty) {
      _initialized = true;
    ...
    try {
      ...
      final opts = AppsFlyerOptions(
        afDevKey: devKey,
        appId: TdbConfig.analyticsAppId,
      ...
      tdbLog('[attr] sdk init OK');
    } catch (err, st) {
      tdbLog('[attr] init error: $err\n$st');
      if (!_attributionReady.isCompleted) _attributionReady.complete({});
      if (!_linkReady.isCompleted) _linkReady.complete();
    }
  }
  ...
  void _handleConversion(dynamic raw) async {
    ...
    tdbLog('[attr] conversion received');
    if (data['af_status'] == 'Organic') {
      await Future.delayed(Duration(seconds: TdbConfig.organicRetrySeconds));
  ...
  Future<Map<String, dynamic>?> _fetchGcdData() async {
    try {
      ...
      final appId = Platform.isIOS ? TdbConfig.analyticsAppId : TdbConfig.bundleId;
      final url = tdbGcdUrl(appId, uid);
      if (url.isEmpty) return null;
      final resp = await tdbHttp.get(
        Uri.parse(url),
        headers: {'authorization': 'Bearer ${TdbConfig.installKey}'},
  ...
  Future<Map<String, dynamic>> buildPayload({
    ...
    body['bundle_id'] = TdbConfig.bundleId;
    body['store_id']  = TdbConfig.platformStoreId;
    body['os']        = Platform.isAndroid ? 'Android' : 'iOS';
    body['locale']    = locale;
    if (pushToken != null && pushToken.isNotEmpty) {
      body['push_token'] = pushToken;
    }
    if (TdbConfig.firebaseNumber.isNotEmpty) {
      body['firebase_project_id'] = TdbConfig.firebaseNumber;
    }

    tdbLog('[attr] payload built');
    return body;
  }
}
