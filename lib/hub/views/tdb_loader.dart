...
import '../infra/tdb_request.dart';
import '../infra/tdb_tap.dart';
import '../infra/tdb_push.dart';
import '../infra/tdb_net.dart';
import '../infra/tdb_store.dart';
import '../infra/tdb_attr.dart';
import '../data/tdb_mode.dart';
import 'tdb_browser.dart';
import 'tdb_offline.dart';
import 'tdb_notify.dart';
...
class TdbLoader extends StatefulWidget {
  ...
  @override
  State<TdbLoader> createState() => _TdbLoaderState();
}

class _TdbLoaderState extends State<TdbLoader> {
  ...
  Future<void> _updateVideo(Orientation o) async {
    final asset = o == Orientation.landscape
        ? 'assets/boot/boot_landscape.mp4'
        : 'assets/boot/boot_portrait.mp4';
    final prev = _player;
  ...
  Future<void> _launch() async {
    ...
    final nativeColdUrl = await TdbTap.consumeTapUrl();
    if (nativeColdUrl != null && nativeColdUrl.isNotEmpty) {
      tdbLog('[ld] native cold-start url → $nativeColdUrl');
      await widget.vault.writeMode(TdbMode.web);
      await widget.vault.consumeOneShotUrl();
      unawaited(_sendAttribution());
      SystemChrome.setEnabledSystemUIMode(SystemUiMode.immersiveSticky);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        _routeToContent(nativeColdUrl, coldStartPush: true);
      });
      return;
    }
  ...
  Future<void> _runFirstLaunch() async {
    ...
    if (reply.granted && reply.destination != null) {
      ...
      await widget.vault.writeMode(TdbMode.game);
      _setPhase(_BootPhase.ready);
      await Future.delayed(const Duration(milliseconds: 400));
      if (!mounted) return;
      _routeToGame();
  ...
  void _routeToGame() {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(builder: (_) => const TdbGameRoot()),
    );
  }
  ...
  String _progressBarImage() {
    switch (_phase) {
      case _BootPhase.idle:    return 'assets/boot/boot_pulse_1.webp';
      case _BootPhase.loading: return 'assets/boot/boot_pulse_2.webp';
      case _BootPhase.ready:   return 'assets/boot/boot_pulse_4.webp';
    }
  }
